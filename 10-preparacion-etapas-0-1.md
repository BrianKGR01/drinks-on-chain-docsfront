...
|---|---|---|---|---|
| 1.1 | **Repositorios en GitHub** `doc-design-system`, `doc-mocks`, `doc-erp-web` bajo `BrianKGR01` (o una organización `drinks-on-chain` si se prefiere; ver §4) | Cliente crea (vacíos, privados o públicos) o autoriza a que los cree el equipo con `gh repo create` desde su sesión | Etapa 0.1, 0.2 y 1 | **Hecho 25-09**: `drinks-on-chain-design-system`, `-mocks`, `-app-template` y `-erp` en la organización `drinks-on-chain`, públicos, con `main` y `dev` |
| 1.2 | **GitHub Packages**: un token personal (PAT clásico) con `read:packages` para instalar `@doc/ui` y `@doc/mocks` en las apps, y `write:packages` para publicar desde local si hace falta. En CI basta el `GITHUB_TOKEN` del propio repositorio | Cliente genera el PAT y lo guarda como secreto `NPM_TOKEN` en cada repo consumidor | Publicar y consumir los paquetes | **Ya no hace falta**: los paquetes se publican como tarball en cada GitHub Release (etiqueta `vX.Y.Z` en `main`) y las apps los instalan por URL |
| 1.3 | **Vercel**: proyectos `drinks-on-chain-erp` (y luego los demás) en el equipo `briankgr01s-projects`, conectados a los repos, con producción desde `main` y previews desde `dev` | Cliente (importa el repo en Vercel) o el equipo con el MCP de Vercel ya conectado | Previews de cada PR | **Hecho 25-09**: https://drinks-on-chain-erp.vercel.app (con mocks) y https://drinks-on-chain-storybook.vercel.app |
...
| 2.3 | CORS para los dominios de Vercel (`*.vercel.app`) y, cuando exista, el dominio real. Hoy solo se ha verificado `localhost` | Backend | **Verificado 25-09**: el preflight admite `https://drinks-on-chain-erp.vercel.app` y `http://localhost:3002`; falta el dominio real cuando exista |
...
|---|---|---|
| 4.1 | **Dónde viven los repos**: cuenta personal `BrianKGR01` (como hoy) o una organización `drinks-on-chain` en GitHub. **Decidido 25-09: organización** (los tres repos existentes se copiaron como forks; conviene transferirlos en lugar de mantener forks) | Crear la organización ahora, antes de que haya seis repos más: permisos por equipo, Packages con ámbito `@drinks-on-chain`, y los colegas de backend pueden verse invitados. Transferir los tres repos actuales es un clic |
| 4.2 | **Ámbito de los paquetes**: `@doc/ui` y `@doc/mocks` (como en los documentos) o `@drinks-on-chain/ui`. **Decidido 25-09: `@drinks-on-chain/*`**; repos con nombre `drinks-on-chain-<sistema>` | Depende de 4.1: con organización, `@drinks-on-chain/*` |
| 4.3 | **Visibilidad**: repos privados o públicos. **Decidido 25-09: públicos** (Vercel Hobby no despliega repos privados de una organización; el SCF pide código abierto) | Privados ahora; abrir `doc-design-system` y `doc-wallet` cuando se presente la candidatura al Stellar Community Fund (pide código abierto) |
